...
class SecondStormCompleteMatrix:
    ...
    def __init__(self):
        # Structural wind speed and angle axis vectors from the ORC Speed Guide
        ...
        # 1. Target Boat Speed Matrix (BTV) [TWA x TWS]
        ...
        # 2. Target Heel Angle Matrix (degrees) [TWA x TWS]
        self.heel_grid = np.array([
            [22.5, 24.2, 21.7, 24.7],  # 60° TWA
        ...
        # 3. Target Reef Matrix (sail-area fraction, 1.0 = full main) [TWA x TWS]
        self.reef_grid = np.array([
            [1.00, 0.95, 0.87, 0.91],  # 60° TWA
        ...
        # 4. Target Flatness Configuration Matrix [TWA x TWS]
        self.flat_grid = np.array([
            [0.93, 0.91, 0.93, 0.61],  # 60° TWA
        ...
        # Generate individual 2D analytical splines for each target state
        self.btv_spline = RectBivariateSpline(self.twa_axis, self.tws_axis, self.btv_grid, kx=3, ky=3)
        self.heel_spline = RectBivariateSpline(self.twa_axis, self.tws_axis, self.heel_grid, kx=3, ky=3)
        self.reef_spline = RectBivariateSpline(self.twa_axis, self.tws_axis, self.reef_grid, kx=3, ky=3)
        self.flat_spline = RectBivariateSpline(self.twa_axis, self.tws_axis, self.flat_grid, kx=3, ky=3)
    ...
    def evaluate_performance(self, twa, tws, stw):
        """Back-compat entry point for callers (e.g. loop3_router.py) that
        only need target boat speed and efficiency."""
